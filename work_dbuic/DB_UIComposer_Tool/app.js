(() => {
  ...
  function deleteScene(sceneId) {
    ...
    if (!scene) return;
    const ok = !window.confirm || window.confirm(`シーン「${scene.name || scene.id}」を削除します。グループとウィンドウは削除されません。よろしいですか？`);
    if (!ok) return;
    runStateMutation("シーン削除", () => {
      state.scenes = ensureScenes().filter(entry => entry.id !== id);
      if (state.activeSceneId === id) state.activeSceneId = "";
      if (selected?.kind === "scene" && selected.sceneId === id) selected = null;
    });
  }
...
`;
  ...
  function renderSceneProperties(scene) {
    ...
    addPropertyButtonRow([
      ...
      { text: "削除", className: "danger", onClick: () => deleteScene(scene.id), help: "シーンだけ削除します。グループとウィンドウは削除されません。" }
    ]);
